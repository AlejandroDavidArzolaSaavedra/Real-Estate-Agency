...
public class AgenciaFile {
                    ...
                    for (k = 0; k < inmobiliaria.getClientes().get(j).getAnuncios().getAnunciosList().size(); k++) {
                        if (inmobiliaria.getClientes().get(j).getAnuncios().getAnunciosList().get(k) instanceof Anuncio) {
                            ...
                            pw.write(anuncio.getInmueble().getTipoInmueble() + "/" + anuncio.getInmueble().getSuperficie() + "/" + anuncio.getInmueble().getNumeroHabitaciones()
                                    ...
                                    + "/" + anuncio.getTipoOperacion() + "/" + anuncio.getPrecio() + "/" + anuncio.getFoto() +
                                        "/" + anuncio.getTitulo()+ "/" + anuncio.getDescripcion()+ "/" + anuncio.getTipoOperacion() +"/\n");
                        }
                    }
                }
            }
    ...
    public void loadFromFile(Inmobiliaria inmobiliaria) {
        try {
            //CatalogoAnuncios catalogo = new CatalogoAnuncios();
            ...
            for (i = 0; i < n; i++) {
                ...
                for (j = 0; j < m; j++) {
                    ...
                    String tipo = s.next();
                    if (tipo.equals("PISO")) {
                        inmueble = new Inmueble(TipoInmueble.PISO, Double.parseDouble(s.next()), Integer.parseInt(s.next()));
                    } else if (tipo.equals("CASA")) {
                        //inmueble = new Inmueble(TipoInmueble.CASA, Double.parseDouble(s.next()), Integer.parseInt(s.next()));
                    } else if (tipo.equals("CHALET")) {
                        //inmueble = new Inmueble(TipoInmueble.CHALET, Double.parseDouble(s.next()), Integer.parseInt(s.next()));
                    } else if (tipo.equals("APARTAMENTO")) {
                        //inmueble = new Inmueble(TipoInmueble.APARTAMENTO, Double.parseDouble(s.next()), Integer.parseInt(s.next()));
                    }
