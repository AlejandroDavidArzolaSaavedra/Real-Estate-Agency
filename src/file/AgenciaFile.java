...
public class AgenciaFile {
    ...
    public void saveToFileCatalogo(Inmobiliaria inmobiliaria) {
        FileWriter fichero = null;
        PrintWriter pw = null;
        try {
            fichero = new FileWriter(catalogo);
            pw = new PrintWriter(fichero);
            int j, k;
            pw.write(inmobiliaria.getCatalogo().size() + "\n");

            CatalogoAnuncios catalogo = null;
            for (j = 0; j < inmobiliaria.getCatalogo().size(); j++) {
                if (inmobiliaria.getCatalogo().get(j) instanceof CatalogoAnuncios) {
                    ...
                    Anuncio anuncio = null;
                    pw.write(catalogo.getAnuncios().size() + "\n");
